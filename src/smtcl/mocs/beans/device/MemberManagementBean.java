...
public class MemberManagementBean implements Serializable {
	 ...
	 * 人员管理接口实例
	 */
	private IMemberService memberService = (IMemberService) ServiceFactory
			.getBean("memberService");// 获取注入;

	/**
	 * 权限接口实例
	 */
	private IOrganizationService organizationService = (IOrganizationService) ServiceFactory
	...
	public MemberManagementBean() {
		...
		for (Map<String, Object> rec : nodes) {
			...
			nodeMap.put((String) rec.get("nodeName"),
					(String) rec.get("nodeName"));
		}
		...
		TUser user = (TUser) FaceContextUtil.getContext().getSessionMap()
	...
	public void changePositionTeam(String id) {
		...
		for (Map<String, Object> rec : teams) {
			if (rec.get("teamName") == null || rec.get("teamName").equals(""))
				continue;
			dictTeam.put((String) rec.get("teamName"), rec.get("teamid")
					.toString());
			teamMap.put((String) rec.get("teamName"),
					(String) rec.get("teamName"));
		}

		// 职位id、名称键值对
		List<Map<String, Object>> positions = memberService.getPositionList(id);
		for (Map<String, Object> rec : positions) {
			if (rec.get("positionName") == null
					|| rec.get("positionName").equals(""))
				continue;
			dictPosition.put((String) rec.get("positionName"),
					rec.get("positionId").toString());
			positionMap.put((String) rec.get("positionName"),
					(String) rec.get("positionName"));
		}
	}
	...
	public void searchList() {
		...
		if (null != results && results.size() > 0)
			results.clear(); // 没有从列表查询到值的话，清空全部数据
	...
	public void onNodeSelect(NodeSelectEvent event) {
		smtcl.mocs.beans.authority.cache.TreeNode currentNode = (smtcl.mocs.beans.authority.cache.TreeNode) event
				.getTreeNode().getData();
		if (!currentNode.isNocheck()) {
			this.nodeStr = currentNode.getNodeId(); // 将单机节点的值赋给当前节点nodeStr
			thisNodeName = StringUtils.getSubString(currentNode.getNodeName(),
					"5");// 获取当前点击的具体树型值
	...
	public void deleteRecord() {
		...
		for (Map<String, Object> rec : selectedData) {
			memberService.setStatus("t_member_info", "ID",
					Integer.valueOf(rec.get("id").toString()));
		}
	...
	public void insertRecord() {
		...
		List<Map<String, Object>> memberTemp = memberService.checkMember(
				insertData.getNo(), nodeStr);
		if (memberTemp.size() == 0 || memberTemp == null) {
			insertData.setNodeid(nodeid);
			if (insertData.getBirthday() != null) {
				Calendar cal = Calendar.getInstance();
				int year = cal.get(Calendar.YEAR);
				int y = Integer.parseInt(StringUtils.formatDate(
						insertData.getBirthday(), 2).substring(0, 4));
				insertData.setAge(year - y);
			}
			if (insertData.getBirthday() != null)
				insertData.setBirthday(StringUtils.convertDate(
						StringUtils.formatDate(insertData.getBirthday(), 2),
						"yyyy-MM-dd"));
			...
			if (positionName != null && !positionName.equals("")) {
				if (dictPosition.get(positionName) != null)
					insertData.setPositionid(Long.valueOf(dictPosition
							.get(positionName)));
			}
	...
	public void updateRecord() {
		...
		List<TMemberInfo> members = memberService.getRecordById(Integer
				.valueOf(newData.get("id").toString()));
		if (members != null && members.size() > 0) {
			...
			tm.setMarriage(null == newData.get("marriage") ? "" : newData.get(
					"marriage").toString());
			...
			if (newData.get("birthday") != null) {
				tm.setBirthday(StringUtils.convertDate(StringUtils.formatDate(
						(Date) newData.get("birthday"), 2), "yyyy-MM-dd"));
				Calendar cal = Calendar.getInstance();
				int year = cal.get(Calendar.YEAR);
			...
			if (positionNameUp != null && !positionNameUp.equals("")) {
				if (dictPosition.get(positionNameUp) != null)
					tm.setPositionid(Long.valueOf(dictPosition
							.get(positionNameUp)));
			}
