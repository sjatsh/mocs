...
public class ResourceServiceImpl extends
	...
	public List<Map<String, Object>> getTMaterialClassByAll() {
		String hql = "select new Map(" + " a.MClassid as id,"
				+ " a.MClassname as name)" + " from TMaterialClass a"
				+ " where MStatus=0";
		return dao.executeQuery(hql);
	}
	 ...
	 * @return
	 */
	public List<Map<String, Object>> getTMaterailTypeInfo(String pid,
			String nodeid, String type, String no, String desc, String status,
			String unit, Date startTime, Date endTime) {
		String hql = "select new Map(" + " a.id as id," + " a.no as no,"
				+ " a.name as name," + " a.norm as norm,"
				+ " a.typeno as typeno,"
				...
				+ " end as className ," + " a.PId as pid,"
				+ " b.MClassname as MClassname)"
				+ " from TMaterailTypeInfo a ,TMaterialClass b"
				+ " where a.PId=b.MClassid " + " and a.nodeId='" + nodeid
				+ "' ";
		...
		if (null != startTime && !startTime.toString().equals("")) {
			hql += " and DATE_FORMAT(a.createTime,'%Y-%m-%d %T')>='"
					+ StringUtils.formatDate(startTime, 2) + "'";
		}
		if (null != endTime && !endTime.toString().equals("")) {
			hql += " and DATE_FORMAT(a.createTime,'%Y-%m-%d %T')<='"
					+ StringUtils.formatDate(endTime, 2) + "'";
		}
