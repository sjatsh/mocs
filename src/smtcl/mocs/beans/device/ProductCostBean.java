...
public class ProductCostBean implements Serializable {
    ...
    public ProductCostBean() {
        ...
        if(lst!=null&&lst.size()>0) {
            partTypeNo = lst.get(0).get("parttypeNo") + "";   //进去默认初始
       ...
       System.out.println("构造方法程序运行时间： "+(endTime-startTime)+"ms");
    ...
    public void searchList(){
        ...
        results = costManageService.queryProcessCostList(1,100, partTypeNo);
        long endTime=System.currentTimeMillis(); //获取结束时间
        System.out.println("查询程序运行时间： "+(endTime-startTime)+"ms");
    }
    ...
    public String toColumJSON() {
        ...
        if(locale.toString().equals("en") || locale.toString().equals("en_US")){
            columns = new String[] {"Labor", "Accessories", "Energy", "Depreciation", "Material"};	 //纵坐标
        }else{
            columns = new String[] {"人工", "辅料", "能源", "折旧", "主料"};	 //纵坐标
        }
    ...
    public String toPieJson(){
        ...
        Locale locale = this.getLocale();
        if(locale.toString().equals("en") || locale.toString().equals("en_US")){
            names = new String[] {"Labor", "Accessories", "Energy", "Depreciation", "Material"};
        }else {
            names = new String[] {"人工", "辅料", "能源", "折旧", "主料"};
        }
        String peopleCost = "0";
    ...
    public String tocolumnAndLineJson() {
        ...
        if(dataMap!=null){
            ...
            Locale locale = this.getLocale();
            if(locale.toString().equals("en") || locale.toString().equals("en_US")){
                columns = new String[] {"Cutting", "Ready", "Auxiliary"};	 //纵坐标
            }else{
                columns = new String[] {"切削", "准备", "辅助"};	 //纵坐标
            }
            try {
                data.put("rowkeys", rows);
                data.put("columnkeys", columns);
    ...
    public String getColumJsonData() {
       ...
       long endTime=System.currentTimeMillis(); //获取结束时间
       System.out.println("分区柱状图程序运行时间： "+(endTime-startTime)+"ms");
