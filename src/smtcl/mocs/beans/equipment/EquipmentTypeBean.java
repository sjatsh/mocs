...
public class EquipmentTypeBean implements Serializable {
	...
	public void MaterailSearch() {
		if ("输入物料名称".equals(search))
			search = null;
		root = equipmentService.getEquTreeNodeOnAll(search, nodeid);
	}
	...
	public void onNodeSelect(NodeSelectEvent event) {
		...
		List<Map<String, Object>> lst = equipmentService
				.getParentNameById(typeId);
	...
	public void addEquType() {
		List<Map<String, Object>> lst = equipmentService
				.getEquTypeRepeat(equTypeObj.getTypecode());
		if (lst.size() == 0) {
			equipmentService.addEquType(this);
			FacesMessage msg = new FacesMessage("设备类型添加", "添加'"
					+ equTypeObj.getEquipmentType() + "'成功！");
			FacesContext.getCurrentInstance().addMessage(null, msg);

		} else {
			FacesMessage msg = new FacesMessage("设备类型添加", "设备类型'"
					+ equTypeObj.getEquipmentType() + "'重复！请重新添加！");
			FacesContext.getCurrentInstance().addMessage(null, msg);
			System.out.println("已经存在，不要重复添加----------->");
		}
		equlist = equipmentService.getEquTypeByClick(nodeid);
		mediumEquipmentTypeInfoModel = new TEquipmentTypeInfoDataModel(equlist);
	...
	public void onDelete() {
		...
		String equid = (this.getSelectedEquipment())[0].get("id").toString();
		equipmentService.delEquType(this);
		try {
			equlist.clear();
			equlist = equipmentService.getEquipmentInfoNowList(equid);
			int num1 = equlist.size();
			num = String.valueOf(num1);
			mediumEquipmentTypeInfoModel = new TEquipmentTypeInfoDataModel(
					equlist);
			...
			for (Map<String, Object> map : lst2) {
				if (map.get("id") != null && !"".equals(map.get("id"))) {
					equTypeMap.put((String) map.get("equipmentType"),
							map.get("id").toString());
				}
			}
			TEquipmenttypeInfo ti = equipmentService
					.getgetEquTypeIdById("-999"); // 最上面1级的上级默认是0
			pid = "-999";
			equTypeObj.setTEquipmenttypeInfo(ti); // /当前的父节点，添加是用
	...
	public void downloadExcel() {
		...
		String[] firstRowValue = new String[] { "设备类型ID", "设备类型名称", "分类编码",
				"制造商型号", "数控系统", "设备类型描述" };
