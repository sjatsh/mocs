...
Ext.define('Sch.locale.En', {
    ...
    l10n        : {
        ...
        'Sch.widget.ExportDialog' : {
            ...
            adjustCols                  : 'Adjust columns width',
            adjustColsAndRows           : 'Adjust columns width and rows height',
...
Ext.define('Sch.mixin.Localizable', {
    ...
    isLocaleApplied     : function () {
        if (!this.self.activeLocaleId) return false;

        for (var ns in Sch.locale.Active) {
            if (this.self.activeLocaleId === Sch.locale.Active[ns].self.getName()) return true;
        }
...
Ext.define("Sch.feature.SchedulerDragZone", {
    ...
    isValidDrop : function (oldResource, newResource, relatedRecords, sourceEvent) {
        ...
        var schedulerView       = this.schedulerView,
            valid               = true,
            indexDiff,
            isTreeStore         = Ext.data.TreeStore && schedulerView.resourceStore instanceof Ext.data.TreeStore,
            viewStore           = isTreeStore ? schedulerView.store : schedulerView.resourceStore,
            newIndex;

        indexDiff               = viewStore.indexOf(oldResource) - viewStore.indexOf(newResource);

        Ext.each(relatedRecords, function (related) {
            newIndex            = viewStore.indexOf(oldResource) - indexDiff;

            if (newIndex < 0 || newIndex >= viewStore.getCount()) {
                valid = false;
                return false;
            }
        });
...
Ext.define("Sch.feature.ResizeZone", {
    ...
    createResizer : function (el, eventRecord, handlePos) {
        ...
        // Ext is doing some weird caching of listeners in the IE
        // we need to duplicate the cache entry
        if (Ext.isIE) {
...
Ext.define("Sch.data.mixin.FilterableTreeStore", {
    ...
    /**
     * This method filters the tree store. It accept an object with the following properties:
     * 
...
A plugin (ptype = 'scheduler_export') for generating PDF/PNG out of a Scheduler panel.
...
Ext.define("Sch.plugin.DragSelector", {
    ...
    onStart : function (e){
        ...
        if (!this.proxy) {
            this.proxy = schedulerView.el.createChild({ cls:'sch-drag-selector' });
        } else {
            this.proxy.show();
        }
    ...
    */
    ...
    zoomToSpan : function (span, config) {
        if (span.start && span.end && span.start < span.end) {
            ...
            var adjustStart  = function (start, mainUnit) {
                return config.adjustStart ? Sch.util.Date.add(start, mainUnit, - config.adjustStart) : start;
            };

            var adjustEnd  = function (end, mainUnit) {
                return config.adjustEnd ? Sch.util.Date.add(end, mainUnit, config.adjustEnd) : end;
            };
            ...
            var currLevel   = Math.floor(this.getCurrentZoomLevelIndex()),
                diffMS      = adjustEnd(span.end, this.timeAxis.mainUnit) - adjustStart(span.start, this.timeAxis.mainUnit),
            ...
            var zoomLevel, preset, unit, zoomed = null, levelToZoom = false, unitToZoom = this.timeAxis.mainUnit;
            ...
            while (level >= 0 && level <= this.zoomLevels.length - 1) {
                ...
                diffMS      = adjustEnd(span.end, unit) - adjustStart(span.start, unit);
            ...
            if (levelToZoom !== false) zoomed = this.zoomToLevel(levelToZoom, true);

            // apply the new timeframe
            var prevAutoAdjust          = this.timeAxis.autoAdjust;
            this.timeAxis.autoAdjust    = false;
            this.setTimeSpan(adjustStart(span.start, unitToZoom), adjustEnd(span.end, unitToZoom));
            this.timeAxis.autoAdjust    = prevAutoAdjust;
