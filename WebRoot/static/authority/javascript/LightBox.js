...
//	if(isIE6){ this._resize(); window.attachEvent("onresize", this._resize); }
	if(isIE){ this._resize(); window.attachEvent("onresize", this._resize); }
	//设置样式
	with(this.Lay.style){
		//设置透明度
...
//	if(isIE6){ window.detachEvent("onresize", this._resize); }
	if(isIE){ window.detachEvent("onresize", this._resize); }
  }
};
...
//	if(isIE6){
  ...
  Show: function(options) {
	//固定定位
	this.Box.style.position = this.Fixed && !isIE ? "fixed" : "absolute";
