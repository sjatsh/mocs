...
public class OrgAjax {
	...
	public String queryOrgTree(@AjaxParameter(name = "async") boolean async,
		...
		try {
			...
			if (async) {
				list = AuthorityUtil.queryAuthOrgTree(userId, pageId,
						"button.view", null, false, null);
	...
	@AjaxMethod
	public String queryOrgChild(@AjaxParameter(name = "nodeId") String nodeId,
			@AjaxParameter(name = "userId") String userId,
		...
		JsonResponseResult jrr = new JsonResponseResult();
		try {
			List<TreeNode> list = AuthorityUtil
					.queryAuthOrgTree(userId, pageId, "button.view", nodeId,
							true, exclude, filter, false);
			for (TreeNode t : list) {
				Organization o = (Organization) t.getBindData();
				if (o != null) {
					o.setManage(AuthorityUtil.judge(userId, o.getOrgId(),
							pageId, "button.manage"));
				}
			}
			jrr.setContent(list);
			jrr.attach("manage", AuthorityUtil.judge(userId, nodeId, pageId,
					"button.manage"));
		} catch (Exception e) {
			jrr.setSucc(false);
			jrr.setMsg(SessionHelper.getText("app.non-date.info"));
	...
	public String saveOrg(
			@AjaxParameter(name = "node", serialize = true) Organization node,
			@AjaxParameter(name = "userId") String userId,
			@AjaxParameter(name = "pageId") String pageId) {
		JsonResponseResult jrr = new JsonResponseResult();
		LoginUser loginUser = SessionHelper.loginUser(HttpContext.current()
				.getSession());
		String s = "";
		String nodeId = null;
		String oldPId = null;
		String newPId = null;
		try {
			ITreeService service = (ITreeService) ServiceFactory
					.getBean("treeService");
			...
			if (StringUtils.isNotBlank(orgId)) {// modify
				...
				if (o == null)
					throw new AuthorityException(
							SessionHelper.getText("org.node.not.exist"));

				if (!o.getParentId().equals(node.getParentId())) {
					s = String.format(" (judge node parent from[%s] to[%s])",
							o.getParentId(), node.getParentId());
					flat = true;
				...
				o.setOrgType(service.get(TypeNode.class, node.getOrgType()
						.getTypeId()));
			...
			LogHelper.log(
					...
					pageId,
					String.format("save org! name:[%s],type:[%s] %s",
							o.getName(), o.getOrgType(), s),
					ILogFinal.LOG_SUCCESS);
		} catch (Exception e) {
			jrr.setSucc(false);
			...
			e.printStackTrace();
			LogHelper.log(loginUser.getUser().getLoginName(), pageId, String
					.format("save org error! name:[%s],type:[%s] %s",
							node.getName(), node.getOrgType(), s),
					ILogFinal.LOG_FAIL);
		}
		return jrr.toJsonString();
	}
	...
	public String delOrg(@AjaxParameter(name = "nodeId") String nodeId,
		...
		try {
			...
			Parameter s = new Parameter("status", new Long(IConsant.INVALID),
					Operator.EQ);
			...
			IOnLineService os = (IOnLineService) ServiceFactory
			...
			if (IConsant.NODE_TYPE_DEV.equals(o.getOrgType().getTypeId())) {
				...
				if (dev != null) {
					if (dev.getSymgEquId() != null) {
						boolean f = false;
						try {
							f = os.unbindDev(loginUser.getUser().getUserId(),
									o.getMesNodeid(), o.getOrgId());
				...
				String h3 = "select 1 from Organization o, Device d where o.orgId = d.nodeId and o.orgType.typeId= '"
						+ IConsant.NODE_TYPE_DEV
						+ "' and d.symgEquId is not null and o.orgId in (select fo.nodeId from FlatOrg fo where fo.ancestorId= :nodeId) ";
				List list_h3 = service.executeQuery(h3,
						new Parameter[] { new Parameter("nodeId", nodeId,
								Operator.EQ) });
				if (!list_h3.isEmpty()) {
					boolean f = false;
					try {
						f = os.unbindNode(loginUser.getUser().getUserId(),
								o.getOrgId());
					} catch (OnLineDevException ode) {
						throw new AuthorityException(ode.getMessage());
					}
			...
			/*******************change by sunjun 2015年12月14日19:41:06*****************/
			String sql2 = "delete from t_equipmentaddinfo where nodeID='"
					+ nodeId + "'";
			service.executeNativeUpdate(sql2);
			String sql = "delete from T_NODES where nodeId='" + nodeId + "'";
			service.executeNativeUpdate(sql);
			
			
			service.executeUpdate(
					"delete from Device o where o.nodeId in ( select fo.nodeId from FlatOrg fo where fo.ancestorId= :nodeId)",
					new Parameter[] { new Parameter("nodeId", nodeId,
							Operator.EQ) });

			service.executeUpdate(
					"delete from Organization o where o.orgId in ( select fo.nodeId from FlatOrg fo where fo.ancestorId= :nodeId)",
					new Parameter[] { new Parameter("nodeId", nodeId,
							Operator.EQ) });
			...
			jrr.setContent(o);
			LogHelper
					.log(loginUser.getUser().getLoginName(), pageId, String
							.format("del org! id:[%s], name:[%s],type:[%s]",
									nodeId, o.getName(), o.getOrgType()),
							ILogFinal.LOG_SUCCESS);
		} catch (Exception e) {
			jrr.setSucc(false);
			if (e.getClass() == AuthorityException.class)
			...
			e.printStackTrace();
			LogHelper.log(loginUser.getUser().getLoginName(), pageId,
					String.format("del org error! id[%s]", nodeId),
					ILogFinal.LOG_FAIL);
		}
		return jrr.toJsonString();
	}
	...
	// (ITreeService)ServiceFactory.getBean("treeService");
	//
	// service.executeUpdate("delete from Organization o where o.orgId in ( select fo.nodeId from FlatOrg fo where fo.ancestorId in (:nodeId))",
	// new Parameter[]{new Parameter("nodeId", ns, Operator.EQ)});
	//
	// //刷新;
	...
	public String queryMachineInfo(
		...
		JsonResponseResult jrr = new JsonResponseResult();
		IOnLineService s = (IOnLineService) ServiceFactory
				.getBean("onlineService");
		try {
	...
	public String symg_show(@AjaxParameter(name = "nodeId") String nodeId,
			@AjaxParameter(name = "mid") String mid) {
		JsonResponseResult jrr = new JsonResponseResult();
		ICommonService s = (ICommonService) ServiceFactory
