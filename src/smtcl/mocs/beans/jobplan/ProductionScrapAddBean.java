...
import smtcl.mocs.model.MaterialDataModel;
...
public class ProductionScrapAddBean{
	...
	private List isCurrentProcess;//是否至工序已加工
	private TProductionScrapInfoModel  tps=new TProductionScrapInfoModel();//生产报废属性
	private IJobPlanService jobPlanService = (IJobPlanService)ServiceFactory.getBean("jobPlanService");
	private IScrapSerice scrapService = (IScrapSerice)ServiceFactory.getBean("scrapService");//注入另外一个数据库的service
	...
	private MaterialDataModel processList;//工时算法 所有数据显示
	...
	private List selectedpl=new ArrayList();//选中工序集合
	public ProductionScrapAddBean(){
		String wy=StringUtils.getUniqueString();//唯一标识
//		String tzdcode="BF_";
//		tps.setTzdCode(tzdcode);
		HttpServletRequest request = (HttpServletRequest)FacesContext.getCurrentInstance().getExternalContext().getRequest();
		IScrapSerice scrapService = (IScrapSerice)ServiceFactory.getBean("scrapService");//注入另外一个数据库的service
		//预加载用户列表
		...
		jobdispatchNoList=jobPlanService2.getJobdispatchList(nodeid,null,jobPlanId);
		...
		if ( null!=request.getParameter("jobdispatchNO")){
			...
			processList=new MaterialDataModel(processNoList);
		}
		//预加载零件列表信息
		partList=scrapService.getTPartTypeInfoByNodeid(nodeid,null);
	}
	...
	public void selectPartChange(){
		jobPlanList=scrapService.getJopPlanByPartId(selectedPartId);
		List<Map<String,Object>> partlist=scrapService.getTPartTypeInfoByNodeid(nodeid,selectedPartId);
	...
	public void OnTreeNodeSelect(NodeSelectEvent event){//工单编号选择事件
		...
		erpTaskNoList=new ArrayList<Map<String,Object>>();
		tps.setEntityName(mapJob.get("taskNO").toString());//erp任务号
	...
	public void SelectCurrentProcess(){
		if(isCurrentProcess.size()>0){//如果当前已经勾选了 是否至工序加工  那么 工时算法里面至工序必须勾选
			...
			for(int i=0;i<ls.length;i++){
				...
				if(!pMap.get("processOrder").toString().equals(tps.getToOperationNum().toString())){
					selectProcess[i]=ls[i];
				}
			}
		}
	...
	public void saveData(String parm){
		 //message="";
		if("1".equals(parm)){
			message=scrapService.getJobdispatchTSXX(jobdispatchNo, onlineProcessId, processScrapNum, materialScrapNum, isCurrentProcess);
		}else{
			if(null!=isConcession&&isConcession.size()>0){
				djgs=(Integer.parseInt(djgs)*0.5)+"";//单件工时
			}
			message=scrapService.saveTProductionScrapInfo(tps, jobdispatchNo, isCurrentProcess, processScrapNum,materialScrapNum, 
					onlineProcessId,processPlanID,selectedPartId,djgs,selectedpl);
			if(message.equals("保存成功")){
				tps=new TProductionScrapInfoModel();
				jobdispatchNo="";
				selectedPartId="";
				jobPlanId="";
				processScrapNum="";
	...
	public TProductionScrapInfoModel getTps() {
		return tps;
	}
	public void setTps(TProductionScrapInfoModel tps) {
		this.tps = tps;
	}
	...
	public MaterialDataModel getProcessList() {
		return processList;
	}
	public void setProcessList(MaterialDataModel processList) {
